...
class BTree <K extends Comparable<K>, V> {
    ...
    public List<V> search(K k) {
        BTreeNode<K, V> node = searchNode(k);
        if(node == null) {
            return null;
        }
        return Arrays
    ...
    public boolean contains(K k) {
        return searchNode(k) != null ? true : false;
    }

    public boolean contains(K key, V value)  {
        BTreeNode<K, V> node = searchNode(key);
        if(node != null) {
            for(Entry<K, List<V>> entry: node.keys) {
    ...
    public BTreeNode searchNode(K k)  {
    ...
    private boolean less(K o1, K o2) {
    ...
    private class BTreeNode <U extends Comparable<U>, W> {
        Entry<U, List<W>> keys[];  // An array of keys
        ...
        public BTreeNode(int t1, boolean leaf1) {
            ...
            keys = (Entry<U, List<W>>[]) new Entry[2*t-1];
            C = (BTreeNode<U, List<W>>[]) new BTreeNode[2*t];
            n = 0;
        }

        private boolean less(U o1, U o2) {
            return o1.compareTo(o2) < 0;
        }

        private boolean greater(U o1, U o2) {
            return o1.compareTo(o2) > 0;
        }

        // A utility function that returns the index of the first key that is
        // greater than or equal to k
        public int findKey(U key) {
            int idx=0;
        ...
        public void removeFromNonLeaf(int idx) {
            ...
            // in C[idx]
            if (C[idx].n >= t) {
                Entry<U, List<W>> pred = getPred(idx);
                keys[idx] = pred;
            ...
            // Recursively delete succ in C[idx+1]
                ...
                Entry<U, List<W>> succ = getSucc(idx);
                keys[idx] = succ;
        ...
        public Entry<U, List<W>> getPred(int idx)
        {
            // Keep moving to the right most node until we reach a leaf
            BTreeNode<U, W> cur=C[idx];
            while (!cur.leaf)
                cur = cur.C[cur.n];
        ...
        public Entry<U, List<W>> getSucc(int idx)
        {

            // Keep moving the left most node starting from C[idx+1] until we reach a leaf
            BTreeNode<U, W> cur = C[idx+1];
            while (!cur.leaf)
                cur = cur.C[0];
...
// into C[idx]
        public void borrowFromPrev(int idx) {
            ...
            BTreeNode<U, W> child=C[idx];
            BTreeNode<U, W> sibling=C[idx-1];
...
// C[idx+1] is freed after merging
        ...
        // function is called
        public void insertNonFull(U key, W value) {
        ...
        public void splitChild(int i, BTreeNode<U, W> y)
        {
            // Create a new node which is going to store (t-1) keys
            // of y
            BTreeNode<U, W> z = new BTreeNode<>(y.t, y.leaf);
            z.n = t - 1;
